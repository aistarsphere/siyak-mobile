...
void main() {
  group('V2ErrorMapper — real backend codes', () {
    test('maps documented codes to enum (frozen contract 2026-07.1)', () {
      ...
      expect(V2ErrorMapper.fromCode('ROOM_ALREADY_STARTED'), V2ErrorCode.roomStarted);
      expect(V2ErrorMapper.fromCode('ROOM_EXPIRED'), V2ErrorCode.roomExpired);
      expect(V2ErrorMapper.fromCode('WEEKLY_RUN_EXPIRED'), V2ErrorCode.weeklyExpired);
      expect(V2ErrorMapper.fromCode('WEEKLY_ALREADY_COMPLETED'),
      ...
      expect(V2ErrorMapper.fromCode('HINT_LIMIT_REACHED'), V2ErrorCode.hintLimit);
      // Account-based V2 codes
      expect(V2ErrorMapper.fromCode('AUTHENTICATION_REQUIRED'),
          V2ErrorCode.authenticationRequired);
      expect(V2ErrorMapper.fromCode('INSUFFICIENT_COINS'),
          V2ErrorCode.insufficientCoins);
      expect(V2ErrorMapper.fromCode('NOT_YOUR_TURN'), V2ErrorCode.notYourTurn);
      expect(V2ErrorMapper.fromCode('STATE_VERSION_CONFLICT'),
          V2ErrorCode.stateVersionConflict);
      expect(V2ErrorMapper.fromCode('GAME_LANGUAGE_LOCKED'),
          V2ErrorCode.gameLanguageLocked);
    });

    test('retry policy follows the contract legend', () {
      expect(const V2Exception(V2ErrorCode.stateVersionConflict).retry,
          V2Retry.resync);
      ...
      expect(const V2Exception(V2ErrorCode.sessionExpired).isAuthFailure, isTrue);
    });
