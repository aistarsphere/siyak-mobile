...
void main() {
  ...
  group('semantic tokens', () {
    test('light and dark palettes are distinct graphite identities', () {
      expect(AppTokens.light.background, const Color(0xFFF4F2EE));
      expect(AppTokens.dark.background, const Color(0xFF1E2024));
    ...
    test('gold is the PRIMARY interaction color with dark text on it', () {
      expect(AppTokens.light.accentPrimary, AppTokens.gold);
      expect(AppTokens.dark.accentPrimary, AppTokens.gold);
      // dark-graphite foreground → readable on gold
      expect(AppTokens.dark.onAccent.computeLuminance(), lessThan(0.2));
      expect(AppTokens.light.onAccent.computeLuminance(), lessThan(0.2));
    });
    ...
    test('SC.applyBrightness swaps the active token set', () {
      ...
      expect(SC.coral, AppTokens.gold); // legacy "coral" name → gold primary
    ...
    test('onColor picks dark text on gold, light text on graphite', () {
      SC.applyBrightness(Brightness.dark);
      expect(SC.onColor(AppTokens.gold).computeLuminance(), lessThan(0.2));
      expect(SC.onColor(const Color(0xFF353A42)).computeLuminance(),
          greaterThan(0.5));
    });
  });
