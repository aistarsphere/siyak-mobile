              ...
              Expanded(
                child: modes.when(
                  ...
                  data: (info) {
                    ...
                    final selCat = ref.watch(_practiceCatProvider) ??
                        ...
                        Wrap(spacing: 8, runSpacing: 8, children: [
                          for (final c in cats)
                            _chip(c.labelFor(lang.code), c.code == selCat,
                                () => ref.read(_practiceCatProvider.notifier).state = c.code),
                        ]),
                        const SizedBox(height: 20),
                        _label('الصعوبة'),
                        const SizedBox(height: 8),
                ...
                child: StatefulBuilder(builder: (context, setLocal) {
                  return SiyagPrimaryButton(
                    ...
                    onTap: () async {
                      ...
                      final code = ref.read(_practiceCatProvider) ??
                          info.playable.first.code;
                      final cat = info.playable
