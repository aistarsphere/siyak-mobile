                      ...
                      child: Column(
                        children: [
                          if (room.amHost)
                            ...
                            Text('بانتظار المضيف لبدء اللعبة',
        ...
        children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            _act(context, Icons.ios_share_rounded, () {
              SharePlus.instance
                  .share(ShareParams(text: 'رمز الغرفة: ${room.joinCode}'));
            }),
          ]),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(
          child: Text(p.isMe ? '${p.label} (أنت)' : p.label,
              style: ST.ar(15, weight: FontWeight.w500)),
        ),
        Icon(p.connected ? Icons.circle : Icons.circle_outlined,
            size: 9, color: p.connected ? SC.emerald : SC.textFaint),
        if (p.isHost) ...[
          const SizedBox(width: 10),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(
                color: SC.coralDim, borderRadius: BorderRadius.circular(999)),
