...
const _base = String.fromEnvironment('CG_V2_BASE',
    defaultValue:
...
void main() {
  ...
  test('profile → weekly → guess → hint → leaderboard (live)', () async {
    final profiles = RemoteProfileRepository(client(), uiLanguage: 'ar');
    final profile =
        await profiles.register(installationId: id, displayName: 'اختبار');
    expect(profile.profileId, isNotNull);
    expect(profile.shortCode, isNotEmpty);

    final weekly = RemoteWeeklyRepository(client(),
    ...
    final run = await weekly.startRun(
        weekId: challenge.weekId, language: GameplayLanguage.arabic);
    ...
    final lb = await weekly.leaderboard(weekId: challenge.weekId, pageSize: 5);
    expect(lb.entries, isNotEmpty);
  }, timeout: const Timeout(Duration(seconds: 60)));

  test('room create → WS snapshot → start event (live)', () async {
    final host = const Uuid().v4();
    final hostClient =
        V2ApiClient(baseUrl: _base, installationIdLoader: () async => host);
    final profiles = RemoteProfileRepository(hostClient, uiLanguage: 'ar');
    final me = await profiles.register(installationId: host, displayName: 'مضيف');
    final rooms =
        RemoteRoomRepository(hostClient, myProfileId: () => me.profileId);
    final room = await rooms.create(
        language: GameplayLanguage.arabic, category: 'general');
    ...
    final gw = RemoteRealtimeGateway(
        socketBase: socketBase, myProfileId: () => me.profileId);
    ...
    expect(events.any((e) => e.type == RoomEventType.snapshot), isTrue,
        reason: 'first WS frame is room.snapshot');
    expect(events.any((e) => e.type == RoomEventType.roomStarted), isTrue,
