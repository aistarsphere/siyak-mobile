...
class WordSuggestions {
  ...
  factory WordSuggestions.fromJson(Map<String, dynamic> j) => WordSuggestions(
        words: parseList(j['suggestions']),
      );

  /// Parse a `[{word: ...}, ...]` list (also tolerates a bare `[String]`).
  static List<String> parseList(dynamic raw) => (raw as List<dynamic>? ?? const [])
      .map((e) => e is String ? e : (e as Map<String, dynamic>)['word'] as String)
      .toList();
}
